...
/**
 * Base class for every special weapon's behaviour. Each weapon gets its own handler
 * extending this class, so no single manager accumulates every weapon's state and
 * logic. {@link WeaponItemManager} owns one instance of each handler and routes
 * public calls to the right one.
 *
 * <p>The base class exposes the shared dependencies every weapon needs: the global
 * cooldown manager, the items config, the armor hooks, plus a few small helpers.
 * Per-weapon state and cleanup live in each subclass.</p>
 */
public abstract class WeaponItemHandler {

    protected final WeaponItemManager manager;
    ...
    protected WeaponItemHandler(WeaponItemManager manager) {
    ...
    /**
     * Called by {@link WeaponItemManager#cleanup()} when a game ends - cancels
     * scheduled tasks, removes placed blocks/entities and clears per-weapon state.
     */
